...
def vectorize(seqs):
	X = np.zeros((len(seqs), SEQ_LEN, n_letters), dtype=np.uint8)
	for i, sequence in enumerate(seqs):
		for t, letter in enumerate(sequence):
			X[i, t, all_letters.find(letter)] = 1
		for t in range(len(sequence), SEQ_LEN):
			X[i, t, all_letters.find("@")] = 1
	return X


def make_letter_indices(seqs):
	X = np.zeros((len(seqs), SEQ_LEN), dtype=np.uint8)
	for i, sequence in enumerate(seqs):
		for t, letter in enumerate(sequence):
			X[i, t] = all_letters.find(letter)
		for t in range(len(sequence), SEQ_LEN):
			X[i, t] = all_letters.find("@")
	return X
...
class Decoder(torch.nn.Module):
	...
	def __init__(self, latent_dim=N_LATENT, gru_hidden_dim=N_HIDDEN):
		...
		self.rnn = nn.GRU(latent_dim, gru_hidden_dim)
...
dataset = TensorDataset(seq_arr, cat_vec)
dataloader = DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=1, pin_memory=USE_CUDA, drop_last=True)
...
for iter in range(N_EPOCHS):
	...
	# for ind in range(ITER_PER_EPOCH):
		...
		# x, y = get_seq(inds)
		if USE_CUDA:
			x, y = (autograd.Variable(x.transpose(0, 1).cuda()), autograd.Variable(y.cuda()).view((-1, )))
		else:
			x, y = (autograd.Variable(x).transpose(0, 1), autograd.Variable(y).view((-1, )))
		...
		decoder.init_hidden(BATCH_SIZE)
		decoder.zero_grad()
