...
class Autoencoder(Module):
    ...
    def __init__(self, input_dim, latent_dim, rnn_hid_dim, n_layers, dropout):
        ...
        self._dropout = dropout[0]

        self.e_rnn = GRU(input_dim, self._hidden_dim, n_layers, bidirectional=True, dropout=self._dropout)
        self.e_final = Linear(self._hidden_dim, self._latent_dim)

        self.d_rnn = GRU(self._latent_dim, self._hidden_dim, n_layers, bidirectional=True, dropout=self._dropout)
    ...
    def description(self):
        return """Simple Autoencoder model
    GRU -> Linear -> latent vector -> GRU -> Linear
    Latent vector size: {0}
    GRU hidden size: {1}
    GRU number of layers: {2}
    GRU dropout: {3}""".format(self._latent_dim, self._hidden_dim, self._n_layers, self._dropout)